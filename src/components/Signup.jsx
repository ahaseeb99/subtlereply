...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    if (!agreeToUpdates) {
      setError("You must agree to receive valuable updates from SUBTLE REPLY.");
      setIsLoading(false);
    ...
    if (
      passwordStrength.text === "Too weak" ||
    ...
    if (!email || !/^[^@]+@[^@]+\.[^@]+$/.test(email)) {
      setError("Please enter a valid email address with exactly one @-sign.");
      setIsLoading(false);
    ...
    try {
      const response = await axios.post(
        "https://subtlereply-staging-088fa1d6c0e4.herokuapp.com/api/auth/register",
        userData,
        {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setError(response.data.msg || "Failed to register");
      }
    } catch (error) {
      if (error.response) {
  ...
  return (
    <div className="bg-hero-pattern bg-cover bg-no-repeat bg-center min-h-screen flex items-center justify-center p-4">
      <div className="bg-custom bg-opacity-80 p-8 rounded-2xl shadow-2xl w-full max-w-xl">
        <h2 className="text-3xl md:text-4xl font-bold text-center text-white mb-6">
          Sign <span className="text-purple-500">up</span>
        </h2>
        <h3 className="text-center text-white mb-6">Welcome!</h3>
        {showSuccessModal && (
          <SuccessModal
            message="Congratulations, your account has been successfully created. Just one quick verification step before you can jump in."
            url="/verify"
          />
        )}
        <form className="space-y-6" onSubmit={handleSubmit} noValidate>
          ...
          <div className="space-y-4 sm:flex sm:flex-col sm:space-y-0 sm:items-start sm:justify-start">
            <div className="flex items-center">
              <input
                id="updates"
                type="checkbox"
                className="text-purple-600 form-checkbox rounded bg-gray-700 border-gray-600 focus:ring-purple-500"
              ...
              <label
                htmlFor="updates"
                className="ml-2 text-gray-500 text-sm sm:ml-4"
              >
                I consent to receive valuable updates from SUBTLE REPLY and
                acknowledge that I can unsubscribe at any time. No spam,
                guaranteed.
              </label>
            </div>
            <div className="flex items-center">
          ...
          <div className="relative flex items-center justify-center mt-6">
            <div
              ...
              style={{ top: "0.5rem" }}
            />
            <span className="relative bg-custom px-4 text-gray-600">
              or continue with
            </span>
          </div>
          <button
